...
def run_bem_nonlinear(u_measured, sources_true, n_sources=4, 
                       optimizer='L-BFGS-B', n_restarts=1) -> ComparisonResult:
    """Run BEM nonlinear inverse solver."""
    from .bem_solver import BEMNonlinearInverseSolver
...
def run_fem_nonlinear(u_measured, sources_true, n_sources=4,
    ...
    # BEM uses [0, 2π], FEM uses [-π, π]
    theta_meas = np.linspace(0, 2*np.pi, len(u_measured), endpoint=False)
    
    # Convert FEM theta to [0, 2π] for interpolation
    ...
    if quick:
        ...
        try:
            result = run_bem_nonlinear(u_measured, sources_true, n_sources=n_sources, 
                                       optimizer='L-BFGS-B', n_restarts=5)
            results.append(result)
            if verbose:
                print(f"  Position RMSE: {result.position_rmse:.4f}")
                print(f"  Time: {result.time_seconds:.2f}s")
        ...
        for opt, restarts in [('L-BFGS-B', 5), ('differential_evolution', 1)]:
            if verbose:
                name = f"{opt} x{restarts}" if restarts > 1 else opt
                print(f"\nRunning BEM Nonlinear ({name})...")
            try:
                result = run_bem_nonlinear(u_measured, sources_true, n_sources=n_sources, 
...
def compare_with_optimal_alpha(sources_true: List[Tuple[Tuple[float, float], float]],
    ...
    if include_nonlinear:
        if verbose:
            print("\n" + "-"*70)
            print("STEP 4: Nonlinear solvers (differential_evolution)")
            print("-"*70)
        
        # BEM Nonlinear
        if verbose:
            print("\nBEM Nonlinear (differential_evolution)...")
        result = run_bem_nonlinear(u_measured, sources_true, n_sources=n_sources,
                                   optimizer='differential_evolution', n_restarts=1)
        results.append(result)
        if verbose:
