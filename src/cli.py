...
Usage:
    python -m inverse_source_package.cli --help
    python -m inverse_source_package.cli solve --method l1 --alpha 1e-4
    python -m inverse_source_package.cli sweep --output results/sweep.json
...
Examples:
  ...
  # Parameter sweep
  python -m inverse_source_package.cli sweep --method l1 --output sweep.json
  
  # Create config file
  python -m inverse_source_package.cli config --template default --output config.json
        """
    )
