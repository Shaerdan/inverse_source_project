#!/usr/bin/env python3
"""
Wrapper for test_rmse_truncated.py (Test B)
=============================================

Runs the truncated RMSE test for one seed with output organisation.
Follows the same pattern as run_statistical_experiment.py.

Usage:
    python run_rmse_truncated_experiment.py --seed 0 --test-case general_random_intensity
    python run_rmse_truncated_experiment.py --seed 42 --random-rho-min
"""
...
def run_experiment(seed, test_case, rho, r_min, r_max, theta_0,
    ...
    print(f"\n{'='*70}")
    print(f"TRUNCATED RMSE TEST (Test B): Seed {seed}")
    ...
    # Import and run
    from test_rmse_truncated import run_single_seed_rmse_truncated
...
def main():
    parser = argparse.ArgumentParser(
        description="Run truncated RMSE test (Test B) for one seed"
    )
    ...
    parser.add_argument('--output-dir', type=str, default='rmse_truncated_results')
